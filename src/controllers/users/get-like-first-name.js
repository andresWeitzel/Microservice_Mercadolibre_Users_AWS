...
//Errors
const GET_LIKE_FIRST_NAME_USERS_BAD_REQUEST_DETAIL =
  'Bad request, could not get paginated list of users according to first name. Try again.';
const GET_LIKE_FIRST_NAME_USERS_ERROR_DETAIL =
  'ERROR in get-like-first-name lambda function.';
//Vars
let userList;
let eventHeaders;
...
module.exports.handler = async (event) => {
  try {
    ...
    //-- start with db query  ---
    userList = await getLikeFirstName(event);
    ...
    switch (userList) {
      ...
      case null:
        return await requestResult(
          BAD_REQUEST_CODE,
          GET_LIKE_FIRST_NAME_USERS_BAD_REQUEST_DETAIL,
        );
      default:
    ...
    msgResponse = GET_LIKE_FIRST_NAME_USERS_ERROR_DETAIL;
