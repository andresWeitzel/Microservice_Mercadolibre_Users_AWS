...
const GENERIC_ERROR_LOG_MESSAGE = "Error in getLikeNickname service function.";
//Validations
 ...
 */
const getLikeNickname = async function (event) {
  try {
    usersList = null;
    //pagination
    pageSizeNro = 5;
    ...
    nicknameParam = await event.pathParameters.nickname;

    validatePathParam = await validatePathParameters(nicknameParam);
    ...
    if (User != null) {
      await User.findAll({
        ...
        where: {
          nickname: {
            [Op.iLike]: `%${nicknameParam}%`, //containing what is entered, less strictmatch
          },
        },
        limit: pageSizeNro,
        offset: pageNro,
