 ...
 */
const getLikeFirstName = async function (event) {
  try {
    usersList = null;
    //pagination
    pageSizeNro = 5;
    ...
    firstName = await event.pathParameters.firstName;

    validatePathParam = await validatePathParameters(firstName);
    ...
    if (User != null) {
      await User.findAll({
        ...
        where: {
          first_name: {
            [Op.iLike]: `%${firstName}%`, //containing what is entered, less strictmatch
          },
        },
        limit: pageSizeNro,
        offset: pageNro,
