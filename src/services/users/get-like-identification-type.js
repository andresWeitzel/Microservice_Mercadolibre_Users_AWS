...
const GENERIC_ERROR_LOG_MESSAGE =
  'Error in getLikeIdentificationType service function. Caused by ';
//Validations
 ...
 */
const getLikeIdentificationType = async function (event) {
  try {
    usersList = null;
    //pagination
    pageSizeNro = 5;
    ...
    identificationTypeParam = await event.pathParameters.identificationType;

    validatePathParam = await validatePathParameters(identificationTypeParam);
    ...
    if (User != null) {
      await User.findAll({
        ...
        where: {
          identification_type: {
            [Op.like]: `%${identificationTypeParam}%`, //containing what is entered, less strictmatch
          },
        },
        limit: pageSizeNro,
        offset: pageNro,
